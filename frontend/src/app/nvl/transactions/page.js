...
export default function NvlTransactionsPage() {
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const [txData, nvlData] = await Promise.all([
    ...
    const selectedMaterial = useMemo(() => {
        if (!formData.nvl_id) return null;
        return materials.find(m => m.id === parseInt(formData.nvl_id));
    }, [formData.nvl_id, materials]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (formData.type === 'OUT' && selectedMaterial) {
                if (selectedMaterial.ton_kho_hien_tai < qty) {
                    throw new Error(`Không đủ hàng xuất kho. Hiện có: ${selectedMaterial.ton_kho_hien_tai} ${selectedMaterial.dvt}`);
                }
            }

            await fetchApi('/api/nvl/transactions', {
                method: 'POST',
                body: JSON.stringify({
                    ...formData,
    ...
    return (
        <div className="page-content">
            ...
            <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                    <h2>Nhập / Xuất Kho Vật Tư (NVL)</h2>
                    <p style={{ color: 'var(--text-muted)', fontSize: 13, marginTop: 4 }}>Lịch sử nhập kho, xuất kho nguyên vật liệu</p>
                </div>
            ...
            <div className="card" style={{ padding: 20 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
                    <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
                        ...
                        <span style={{ color: 'var(--text-muted)', fontSize: 13 }}>{filteredTransactions.length} phiếu phát sinh</span>
                    </div>
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                        <div style={{ position: 'relative' }}>
                            <Search size={16} style={{ position: 'absolute', left: 10, top: 10, color: 'var(--text-muted)' }} />
                            <input className="form-input" style={{ paddingLeft: 32, width: 250 }}
                                placeholder="Tìm kiếm phiếu, vật tư..." value={searchTerm}
                    ...
                    <table className="table">
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                <tr><td colSpan={10} style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>Không tìm thấy phiếu nhập xuất nào</td></tr>
                                ...
                                <tr key={t.id}>
                                    ...
                                    <td style={{ fontWeight: 700 }}>{t.ma_nvl}</td>
                                    <td style={{ fontWeight: 500 }}>{t.ten_nvl}</td>
                                    <td style={{ 
                                        textAlign: 'right', 
                                        fontWeight: 700, 
                    ...
                    <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 550 }}>
                        ...
                        <form onSubmit={handleSubmit}>
                            <div className="form-group">
                                <label className="form-label">Chọn loại phiếu</label>
                                <div style={{ display: 'flex', gap: 20 }}>
                                    <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
                                        <input type="radio" name="tx_type" checked={formData.type === 'IN'}
                                            onChange={() => setFormData({ ...formData, type: 'IN' })} />
                                        <span className="badge badge-success" style={{ padding: '6px 12px', fontSize: 13 }}>NHẬP KHO (+)</span>
                                    </label>
                                    <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
                                        <input type="radio" name="tx_type" checked={formData.type === 'OUT'}
                                            onChange={() => setFormData({ ...formData, type: 'OUT' })} />
                                        <span className="badge badge-danger" style={{ padding: '6px 12px', fontSize: 13 }}>XUẤT KHO (-)</span>
                                    </label>
                                </div>
                            </div>
                            ...
                            <div className="form-group">
                                ...
                                <select className="form-select" value={formData.nvl_id}
                                    onChange={e => setFormData({ ...formData, nvl_id: e.target.value })} required>
                                    <option value="">-- Chọn NVL --</option>
                                    {materials.map(m => (
                                        <option key={m.id} value={m.id}>
                                            {m.ma_nvl} - {m.ten_nvl} ({m.quy_cach ? `${m.quy_cach}, ` : ''}Tồn: {m.ton_kho_hien_tai} {m.dvt})
                                        </option>
                                    ))}
                                </select>
                            </div>
                            ...
                            {selectedMaterial && (
                                <div style={{ 
                                    ...
                                    <div><strong>Đơn vị tính:</strong> {selectedMaterial.dvt}</div>
                                    <div><strong>Tồn hiện tại:</strong> <span style={{ color: '#059669', fontWeight: 700 }}>{selectedMaterial.ton_kho_hien_tai}</span></div>
                                </div>
                            )}
                            ...
                            <div className="form-group">
                                <label className="form-label">Số phiếu / Số chứng từ (Nếu có)</label>
                                <input className="form-input" placeholder="Ví dụ: PN-001, PX-010"
                            ...
                            <div className="form-group">
                                ...
                                <textarea className="form-input" placeholder="Lý do xuất, hóa đơn kèm theo..." rows={2}
