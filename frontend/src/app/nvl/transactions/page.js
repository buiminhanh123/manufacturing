...
export default function NvlTransactionsPage() {
    ...
    const [ticketForm, setTicketForm] = useState({
        type: 'IN_NVL', // 'IN_NVL' or 'OUT_NVL'
        date: new Date().toISOString().split('T')[0],
        notes: '',
        items: [{ nvl_id: '', quantity: '', notes: '' }]
    });
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const [ticketData, txData, nvlData] = await Promise.all([
                fetchApi('/api/phieu?type_prefix=NVL'),
                fetchApi('/api/nvl/transactions/all'),
                fetchApi('/api/nvl')
            ]);
            setTickets(ticketData || []);
            setTransactions(txData || []);
            setMaterials(nvlData || []);
        } catch (err) {
    ...
    useEffect(() => {
        if (hasPermission('nvl-transactions')) {
            loadData();
        }
    }, []);
    ...
    const nvlOptions = useMemo(() => {
        return materials.map(m => ({
            id: m.id,
            label: `${m.ma_nvl} - ${m.ten_nvl}`,
            info: `Quy cách: ${m.quy_cach || 'N/A'} | Tồn kho: ${m.ton_kho_hien_tai} ${m.dvt}`
        }));
    }, [materials]);
    ...
    const filteredTickets = useMemo(() => {
        let list = tickets;
        if (filterType) {
            const fullType = filterType === 'IN' ? 'IN_NVL' : 'OUT_NVL';
    ...
    const filteredTransactions = useMemo(() => {
        ...
        if (searchTerm) {
            const term = searchTerm.toLowerCase();
            list = list.filter(t =>
                (t.ten_nvl || '').toLowerCase().includes(term) ||
                (t.ma_nvl || '').toLowerCase().includes(term) ||
    ...
    const handleAddItem = () => {
        setTicketForm(prev => ({
            ...prev,
            items: [...prev.items, { nvl_id: '', quantity: '', notes: '' }]
        }));
    };
    ...
    const handleCreateSubmit = async (e) => {
        ...
        try {
            // Validate items
            if (ticketForm.items.some(item => !item.nvl_id || !item.quantity)) {
                throw new Error('Vui lòng chọn vật tư và số lượng cho tất cả các dòng');
            }

            // Check stock if OUT
            if (ticketForm.type === 'OUT_NVL') {
                for (const item of ticketForm.items) {
                    const mat = materials.find(m => m.id === parseInt(item.nvl_id));
                    const qty = parseFloat(item.quantity);
                    if (mat && mat.ton_kho_hien_tai < qty) {
                        throw new Error(`Vật tư '${mat.ten_nvl}' không đủ tồn kho. Tồn hiện tại: ${mat.ton_kho_hien_tai}`);
                    }
                }
            }
            ...
            toast('Ghi nhận phiếu kho thành công');
    ...
    const handleDeleteTicket = async (id, code) => {
        if (!confirm(`Bạn có chắc chắn muốn xóa phiếu kho '${code}' không? Toàn bộ số lượng trong phiếu sẽ được hoàn lại (revert) vào tồn kho.`)) {
            return;
        }

        try {
            await fetchApi(`/api/phieu/${id}`, { method: 'DELETE' });
            toast('Đã xóa phiếu kho và cập nhật lại tồn kho');
            loadData();
        } catch (err) {
            toast(err.message, 'error');
        }
        ...
        return (
            <div className="page-content">
                <div className="card" style={{ padding: '40px', textAlign: 'center' }}>
                    Từ chối truy cập. Bạn không có quyền xem giao dịch NVL.
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="page-content">
            ...
            <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                    <h2>Quản lý Nhập / Xuất Kho Vật Tư (NVL)</h2>
                </div>
                <div style={{ display: 'flex', gap: 10 }}>
                    <button className="btn btn-primary" onClick={() => handleOpenAddModal('IN_NVL')}
                        style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#10b981', borderColor: '#10b981' }}>
                        <ArrowDownLeft size={16} /> Tạo phiếu nhập
                    </button>
                    <button className="btn btn-primary" onClick={() => handleOpenAddModal('OUT_NVL')}
                        style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#ef4444', borderColor: '#ef4444' }}>
                        <ArrowUpRight size={16} /> Tạo phiếu xuất
                    </button>
                </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '20px' }}>
                ...
                <div style={{
                    ...
                    <div>
                        ...
                        <div style={{ fontSize: '20px', fontWeight: 700, color: '#fff', marginTop: '4px' }}>
                            {tickets.filter(t => t.type === 'IN_NVL').length} <span style={{ fontSize: '13px', fontWeight: 'normal', color: 'rgba(255, 255, 255, 0.75)' }}>phiếu</span>
                        </div>
                    </div>
                </div>
                ...
                <div style={{
                    ...
                    <div>
                        ...
                        <div style={{ fontSize: '20px', fontWeight: 700, color: '#fff', marginTop: '4px' }}>
                            {tickets.filter(t => t.type === 'OUT_NVL').length} <span style={{ fontSize: '13px', fontWeight: 'normal', color: 'rgba(255, 255, 255, 0.75)' }}>phiếu</span>
                        </div>
                    </div>
                </div>
                ...
                <div style={{
                    ...
                    <div>
                        ...
                        <div style={{ fontSize: '20px', fontWeight: 700, color: '#fff', marginTop: '4px' }}>
                            {transactions.filter(t => t.type === 'IN').reduce((acc, curr) => acc + parseFloat(curr.quantity || 0), 0).toLocaleString(undefined, { maximumFractionDigits: 2 })} <span style={{ fontSize: '13px', fontWeight: 'normal', color: 'rgba(255, 255, 255, 0.75)' }}>vật tư</span>
                        </div>
                    </div>
                </div>
                ...
                <div style={{
                    ...
                    <div>
                        ...
                        <div style={{ fontSize: '20px', fontWeight: 700, color: '#fff', marginTop: '4px' }}>
                            {transactions.filter(t => t.type === 'OUT').reduce((acc, curr) => acc + parseFloat(curr.quantity || 0), 0).toLocaleString(undefined, { maximumFractionDigits: 2 })} <span style={{ fontSize: '13px', fontWeight: 'normal', color: 'rgba(255, 255, 255, 0.75)' }}>vật tư</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="card" style={{ padding: 20 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                        <div style={{ position: 'relative' }}>
                            ...
                            <input className="form-input" style={{ paddingLeft: 32, width: 280 }}
                                placeholder={activeTab === 'tickets' ? "Tìm theo mã phiếu, ghi chú..." : "Tìm theo mã phiếu, vật tư, ghi chú..."}
                ...
                {activeTab === 'details' && (
                    <div className="table-wrapper" style={{ height: 'calc(100vh - 340px)', overflowY: 'auto' }}>
                        <table className="table" style={{ fontSize: '13px' }}>
                            <thead>
                                <tr>
                                    ...
                                    <th style={{ width: 80, whiteSpace: 'nowrap' }}>Mã NVL</th>
                                    <th>Tên vật tư</th>
                                    <th style={{ textAlign: 'right', width: 90, whiteSpace: 'nowrap' }}>Số lượng</th>
                                    <th style={{ textAlign: 'center', width: 70, whiteSpace: 'nowrap' }}>ĐVT</th>
                            ...
                            <tbody>
                                {loading ? (
                                    ...
                                    <tr key={t.id}>
                                        ...
                                        <td style={{ whiteSpace: 'nowrap' }}>{t.date}</td>
                                        <td style={{ whiteSpace: 'nowrap' }}>{t.ma_nvl}</td>
                                        <td>{t.ten_nvl}</td>
                                        <td style={{
                                            textAlign: 'right',
                                        ...
                                        <td style={{ textAlign: 'center', whiteSpace: 'nowrap' }}>
                                            <span className="badge badge-secondary">{t.dvt}</span>
                                        </td>
                        ...
                        <form onSubmit={handleCreateSubmit} style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                            <div className="modal-body" style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 10, overflow: 'hidden', paddingRight: 4 }}>
                                ...
                                <div className="form-row" style={{ flexShrink: 0 }}>
                                    <div className="form-group">
                                        ...
                                        <input className="form-input" readOnly style={{ background: '#f3f4f6', fontWeight: 600 }}
                                            value={ticketForm.type === 'IN_NVL' ? 'NHẬP KHO (IN)' : 'XUẤT KHO (OUT)'} />
                                    </div>
                                    <div className="form-group">
                                ...
                                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
                                    ...
                                    <div className="table-wrapper" style={{ flex: 1, overflowY: 'auto', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-md)' }}>
                                        <table className="table" style={{ minWidth: 600, border: 'none' }}>
                                            <thead>
                                                <tr>
                                                    <th>Nguyên vật liệu * (Gõ để tìm kiếm)</th>
                                                        ...
                                                        <tr key={index}>
                                                            <td style={{ verticalAlign: 'top', padding: '6px 8px' }}>
                                                                <AutocompleteSelect
                                                                    options={nvlOptions}
                                                                    value={item.nvl_id}
                                                                    onChange={val => handleItemChange(index, 'nvl_id', val)}
                                                                    placeholder="Tìm mã hoặc tên vật tư..."
                                                                />
                                                                {mat && (
                                                                    <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4, paddingBottom: 2 }}>
                                                                        Hiện có: <strong style={{ color: 'var(--accent)' }}>{mat.ton_kho_hien_tai}</strong> {mat.dvt}
                                                                    </div>
                                                                )}
                                                            </td>
                                                            <td style={{ verticalAlign: 'top', padding: '6px 8px' }}>
                                ...
                                <div className="form-group" style={{ flexShrink: 0, marginTop: 4 }}>
                                    ...
                                    <textarea className="form-input" placeholder="Lý do nhập/xuất kho, tên nhà cung cấp..." rows={2}
                            ...
                            <div className="modal-footer" style={{ flexShrink: 0, marginTop: 8, paddingTop: 12, borderTop: '1px solid var(--border-color)' }}>
                                ...
                                <button className="btn btn-primary" type="submit"
                                    style={{ background: ticketForm.type === 'IN_NVL' ? '#10b981' : '#ef4444', borderColor: ticketForm.type === 'IN_NVL' ? '#10b981' : '#ef4444' }}>
                                    Xác nhận lập phiếu
                                </button>
                            </div>
                        </form>
                ...
                <div className="modal-overlay" onClick={() => setShowViewModal(false)}>
                    <div className="modal" onClick={e => e.stopPropagation()} style={{ width: '80vw', height: '95vh', maxWidth: 'none', display: 'flex', flexDirection: 'column', padding: 24, overflow: 'hidden' }}>
                        ...
                        <div className="modal-body" style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 10, overflow: 'hidden', paddingRight: 4 }}>
                            ...
                            {/* Row 2: Danh sách NVL */}
                            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
                                <h4 style={{ marginBottom: 10, flexShrink: 0 }}>Danh sách vật tư</h4>
                                <div className="table-wrapper" style={{ flex: 1, overflowY: 'auto', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-md)' }}>
                                    <table className="table" style={{ border: 'none' }}>
                                        <thead>
                                            <tr>
                                                ...
                                                <th style={{ textAlign: 'center', width: 80 }}>ĐVT</th>
                                                <th>Ghi chú dòng</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {selectedTicket.items.map((item, idx) => (
                                                <tr key={item.id}>
                                                    <td style={{ color: 'var(--text-muted)' }}>{idx + 1}</td>
                                                    <td style={{ fontWeight: 700 }}>{item.ma_nvl}</td>
                                                    <td>{item.ten_nvl}</td>
                                                    <td style={{
                                                        textAlign: 'right',
                                                        fontWeight: 700,
                                                        color: selectedTicket.type === 'IN_NVL' ? '#059669' : '#dc2626'
                                                    }}>
                                                        {selectedTicket.type === 'IN_NVL' ? '+' : '-'}{item.quantity}
                                                    </td>
